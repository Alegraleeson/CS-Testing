...
public class MissingNumberPerformanceTest {
    ...
    @Test(timeout=100)
    public void testMissingNumberStream_Performance() {
        for (int i = 0; i < 1000000; i += 1) {
            mn.missingNumberStream(missingMax, max);
            mn.missingNumberStream(missingNone, max);
            mn.missingNumberStream(missingSeven, max);
        }
    }
